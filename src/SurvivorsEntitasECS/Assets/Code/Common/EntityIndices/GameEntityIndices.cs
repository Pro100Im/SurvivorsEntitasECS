...
namespace Code.Common.EntityIndices
{
    public class GameEntityIndices : IInitializable
    {
        ...
        {
            return new StatKey(
              (component as TargetId)?.Value ?? entity.targetId.Value,
              (component as StatChange)?.Value ?? entity.statChange.Value);
        }
        ...
        private StatusKey GetTargetStatusKey(GameEntity entity, IComponent component)
        {
            return new StatusKey(
              (component as TargetId)?.Value ?? entity.targetId.Value,
              (component as StatusTypeIdComponent)?.Value ?? entity.statusTypeId.Value);
        }
    }
