...
namespace Code.Gameplay.Features.EffectApplication.Systems
{
    public class ApplyStatusesOnTargetsSystem : IExecuteSystem
    {
        ...
        public void Execute()
        {
            ...
            {
                foreach(int targetId in entity.targetBuffer.Value)
                {
                    foreach(StatusSetup setup in entity.statusSetups.Value)
                    {
        ...
        {
            return entity.hasProducerId ? entity.producerId.Value : entity.id.Value;
        }
    }
}